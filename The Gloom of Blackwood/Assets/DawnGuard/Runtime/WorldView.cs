...
namespace DawnGuard.Unity
{
    public sealed class WorldView : MonoBehaviour
    {
        ...
        public void Initialize(GameRoot root,GameCatalog catalog)
        {
            ...
            factory.Part(world,"Ground",new Vector3(8,-.2f,8),new Vector3(17,.4f,17),new Color(.28f,.3f,.25f));
            foreach(var c in root.Game.Rules.blockedCells)
                factory.Part(world,"Rubble",new Vector3(c.x+.5f,.4f,c.z+.5f),new Vector3(.96f,.8f,.96f),new Color(.4f,.4f,.38f));
            ...
            for(int i=0;i<=16;i++)
            {
                factory.Part(grid.transform,"Grid X",new Vector3(i,.012f,8),new Vector3(.018f,.015f,16),new Color(.43f,.46f,.4f));
                factory.Part(grid.transform,"Grid Z",new Vector3(8,.012f,i),new Vector3(16,.015f,.018f),new Color(.43f,.46f,.4f));
            }
        ...
        public void Refresh()
        {
            ...
            cameraView.orthographicSize=Mathf.Max(11.2f,10f/Mathf.Max(.4f,cameraView.aspect));
            bool night=game.Phase==GamePhase.Night || game.Phase==GamePhase.Defeat;
            sun.intensity=Mathf.Lerp(sun.intensity,night ? .4f : 1.1f,blend*.25f);
            sun.color=night ? new Color(.55f,.7f,1) : new Color(1,.88f,.7f);
            grid.SetActive(game.Phase==GamePhase.Day && !root.Completed);
            ...
            foreach(var e in game.Enemies)
            {
                ...
                if(!enemies.TryGetValue(e.instanceId,out go))
                {
                    ...
                    enemies.Add(e.instanceId,go); enemyIds.Add(e.instanceId,e.definitionId);
                }
            ...
            drone.transform.position=Vector3.Lerp(drone.transform.position,new Vector3(game.DroneX,2.2f,game.DroneZ),blend);
        ...
        {
            string kind=enemyIds[id];
            Stack<GameObject> stack;
            if(!pool.TryGetValue(kind,out stack)) { stack=new Stack<GameObject>(); pool.Add(kind,stack); }
            enemies[id].SetActive(false); stack.Push(enemies[id]); enemies.Remove(id); enemyIds.Remove(id);
        }
