...
namespace DawnGuard.BlackwoodV2
{
    public sealed class BlackwoodHud : MonoBehaviour
    {
        ...
        public void Refresh()
        {
            ...
            float max=g.Shelter==null ? 600 : g.MaxHealth(g.Shelter);
            ...
            if(b!=null)
            {
                var d=g.Rules.Building(b.definitionId); int fix=Mathf.CeilToInt((g.MaxHealth(b)-b.health)*.15f),up=d.upgradeCost*b.level;
            ...
            researchLabel.text=g.Tech>0 ? "ТЕСЛА ОТКРЫТА" : "ОТКРЫТЬ ТЕСЛУ  /  120";
            research.interactable=g.Tech==0 && g.HasPoweredLab() && g.Wallet.Credits>=120;
            droneLabel.text=g.DroneLevel>=3 ? "ДРОН: МАКС. УРОВЕНЬ" : "ДРОН УР. "+(g.DroneLevel+1)+"  /  "+(100*g.DroneLevel);
            drone.interactable=g.DroneLevel<3 && g.HasPoweredLab() && g.Wallet.Credits>=100*g.DroneLevel;
