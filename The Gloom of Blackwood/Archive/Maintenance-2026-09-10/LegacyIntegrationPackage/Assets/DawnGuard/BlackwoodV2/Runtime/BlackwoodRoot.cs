    ...
    public sealed class BlackwoodRoot : MonoBehaviour
    {
        ...
        private void Start()
        {
            try
            {
                ...
                save=new SaveService(Path.Combine(Application.persistentDataPath,"blackwood-v2.json"));
        ...
        public void SelectBuild(string id)
        {
            ...
            Message=id=="camp" ? "Лагерь: +45 за рассвет. Окупаемость 4 выплаты. Осталось выплат: "+(Game.Rules.waves.Length-Game.Day) : "Выбрано: "+Game.Rules.Building(id).title+". Нажмите свободную клетку.";
        }
